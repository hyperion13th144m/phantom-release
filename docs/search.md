...
拒絶理由・優先権・データ種別で、件数を見ながら絞り込める。

「データ種別」は文書の取り込み元で、電子データ（XML）からのものが `xml`、
XML が残っておらず HTML から取り込んだものが `html` になる。
...
同じ並びの「印刷用」から印刷用ページを別タブで開ける
（URL の末尾に `/print` を付けたものと同じ）。
